...
public class Matcher {
    ...
    public boolean matchRelationship(net.cloudml.core.Relationship kr, org.cloudml.core.Relationship cr) {
        if (kr != null && cr != null) {
            return cr.getRequiredEnd().getName().equals(kr.getRequiredPort().getName())
                    && cr.getProvidedEnd().getName().equals(kr.getProvidedPort().getName());
        }
        return false;
    }
    ...
    public boolean matchRelationshipInstance(net.cloudml.core.RelationshipInstance kri, org.cloudml.core.RelationshipInstance cri) {
        if (kri != null && cri != null) {
            return cri.getRequiredEnd().getName().equals(kri.getRequiredPortInstance().getName())
                    && cri.getProvidedEnd().getName().equals(kri.getProvidedPortInstance().getName());
        }
        return false;
    }
    ...
    public boolean matchExecuteInstance(net.cloudml.core.ExecuteInstance kei, org.cloudml.core.ExecuteInstance ei) {
        if (kei != null && ei != null) {
            ...
            pepis.add(ei.getProvidedEnd());
            return kei.getName().equals(ei.getName())
                    && matchProvidedExecutionPlatformInstances(kpepis, pepis)
                    && matchRequiredExecutionPlatformInstance(kei.getRequiredExecutionPlatformInstance(), ei.getRequiredEnd());
        }
        return false;
    }
}
